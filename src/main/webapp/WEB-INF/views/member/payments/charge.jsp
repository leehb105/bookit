...
<p>
<span>내 북토리: <span id="myCash"><fmt:formatNumber type="number" maxFractionDigits="3" value="${loginMember.cash}" /></span>토리</span>
</p>
...
<script>
	...
	function requestPay() {
		...
		IMP.request_pay({
			...
			name: '북토리 ' + ($('input[name="amount"]:checked').val() / 1000) + ',000토리 (+' + $('input[name="amount"]:checked').parent().next().next().data('bonus') + '토리)', // 상품이름(회원의 이름 x)
			...
			if(res.success) {
				...
				$.ajax({
					...
					console.log(data);
					alert(data + '토리 충전 성공')
					document.querySelector('#myCash').innerHTML = ( Number((document.querySelector("#myCash").innerHTML.replace(/[^0-9.-]+/g, ""))) + Number(data)).toLocaleString("ko-KR");
					// 북토리 총액 표시하기
				}).fail(function() {
					console.log('fail');
				});
